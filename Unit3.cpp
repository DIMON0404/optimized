...
reserv = "Оптимізацію можливо здійснити за рахунок операцій, які мають резерв у часі:";
bool point = false;
...
moved = "Оптимізацію здійснюємо переміщенням ";
point = false;
for (int i = 0; i < 20; i++)
if (move[i] > 0)
{
 ...
 moved += " " + IntToStr(i + 1) + " операції на " + IntToStr(move[i]);
 if (move[i] == 1) moved += " день";
 else
 if (move[i] < 5) moved += " дні";
 else moved += " днів";
}
moved += '.';
